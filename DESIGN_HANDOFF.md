...
> Moved out of `README.md` on 2026-08-18 so the README can serve its usual
> purpose (project overview, setup, structure). This file is the original
> visual design handoff, unchanged. See [`ARCHITECTURE.md`](ARCHITECTURE.md)
> for how the theme is actually implemented in code.
...
- أيقونات: lucide_icons_flutter بدل Material rounded الحالية.

## Files
- `AK Cars — GTD Mockups.dc.html` — كل الشاشات (افتحه بالمتصفح؛ الجولات 3–4 أعلى الصفحة هي المعتمدة، والمعرّفات `#3a…#4d` روابط داخلية). Local working file, gitignored — not in this repo.
- `ios-frame.jsx`، `image-slot.js` — مكوّنات عرض للنموذج فقط، لا تُنقل للتطبيق. Local working files, gitignored — not in this repo.
